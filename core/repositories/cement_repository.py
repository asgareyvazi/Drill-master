"""Persistence + historical reconstruction for Cement job-volume runs.

DrillMaster's THIRD calculation-history repository. Like the Casing repository,
it is an INDEPENDENT concrete implementation that follows the same principles as
the Torque & Drag repository (single ``session_scope`` unit of work; a detached,
reload-safe ``SavedCementCalculation``; whole-result verification via the shared
engine-agnostic core) but does not share code with either beyond the
verification core, because cement's snapshot/reconstruction are engine-specific.
"""
...
from core.database import CementCalculationRecord
from core.engineering.calculation_verification import (
    VERIFY_UNREADABLE,
    VerificationOutcome,
    classify_verification,
)
from core.engineering.cement_persistence import (
...
@dataclass
class SavedCementCalculation:
    """A detached, reload-safe view of one persisted cement job-volume run.
    ...
    def verify(self, current_method: Optional[str] = None) -> VerificationOutcome:
        ...
        this record or the stored result. Compares the ENTIRE result, including
        the nested ``lead``/``tail`` legs and the ``stacked_hydrostatic`` layer
        list (via the shared deep numeric diff).
        """
...
class CementCalculationRepository(BaseRepository):
    """Save / load / reconstruct Cement job-volume calculation history."""
    ...
    def save_run(
        ...
        *,
        ...
        """Persist ONE cement run as an immutable historical record; returns id.
        ...
        with self.db.session_scope() as session:
            record = CementCalculationRecord(
                well_id=well_id,
                label=label or "",
                method=method,
                ...
                slurry_volume_bbl=summary.get("slurry_volume_bbl"),
                annular_with_excess_bbl=summary.get("annular_with_excess_bbl"),
                displacement_volume_bbl=summary.get("displacement_volume_bbl"),
                total_pump_bbl=summary.get("total_pump_bbl"),
                sacks=summary.get("sacks"),
                hydrostatic_psi=summary.get("hydrostatic_psi"),
    ...
    def get(self, calc_id: int) -> Optional[SavedCementCalculation]:
        with self.db.session_scope() as session:
            row = session.get(CementCalculationRecord, calc_id)
            return self._to_saved(row) if row else None

    def all(self) -> List[SavedCementCalculation]:
        """All persisted runs, newest first (deterministic: created_at, id)."""
        with self.db.session_scope() as session:
            rows = (
                session.query(CementCalculationRecord)
                .order_by(
                    CementCalculationRecord.created_at.desc(),
                    CementCalculationRecord.id.desc(),
                )
                .all()
            )
    ...
    def count(self) -> int:
        with self.db.session_scope() as session:
            return session.query(CementCalculationRecord).count()

    @staticmethod
    def _to_saved(row: CementCalculationRecord) -> SavedCementCalculation:
        return SavedCementCalculation(
            id=row.id,
            label=row.label or "",
            method=row.method,
