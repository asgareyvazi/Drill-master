"""Read-only Casing-strength calculation-history browser.

The casing counterpart of ``TorqueDragHistoryDialog``: it lists persisted
casing-strength runs, shows the frozen historical details of a selected run, and
offers an observational "Verify" action that recomputes the run from its own
...
It owns no engineering logic — it delegates to ``CasingCalculationRepository``
and the Qt-free persistence/verification domain. (It intentionally does not
share widget code with the T&D dialog: the two show different engineering
fields, and a premature shared dialog base would hide those differences.)
"""
...
def build_history_rows(saved_list) -> List[dict]:
    """Qt-free view-model: turn SavedCasingCalculation objects into row dicts."""
    rows = []
    for s in saved_list:
        ...
        rows.append({
            ...
            "yield": params.get("yield_psi"),
            "burst": s.summary.get("burst_rating_psi"),
            "collapse": s.summary.get("collapse_rating_psi"),
            "tension": s.summary.get("pipe_body_yield_lbf"),
            "well_id": s.well_id,
        })
    return rows


class CasingHistoryDialog(QDialog):
    """Browse + inspect + verify persisted casing-strength runs (read-only)."""
    ...
    def __init__(self, repository, current_method: Optional[str] = None,
        ...
        self._saved: List = []
        self.setWindowTitle("Casing Strength — Calculation History")
        self.resize(940, 560)
        self._build_ui()
        self.reload()
    ...
    def _build_ui(self):
        ...
        header = QLabel(
            "Persisted casing-strength runs. Each row is an immutable historical "
        ...
        self.table.setHorizontalHeaderLabels(
            ["Saved", "OD (in)", "Grade", "Burst (psi)", "Collapse (psi)",
             "Body yield (lbf)"])
    ...
    def reload(self):
        ...
        for i, row in enumerate(rows):
            cells = [
                row["when"], _fmt(row["od"], 3), row["grade"],
                _fmt(row["burst"], 0), _fmt(row["collapse"], 0),
                _fmt(row["tension"], 0),
            ]
    ...
    def _describe(self, s) -> str:
        ...
        lines = [
            "IDENTITY",
            f"  Type            : Casing Strength ({s.label or 'run'})",
            ...
            "",
            "INPUT (frozen — reconstructs the calculation without any catalog)",
            f"  OD              : {_fmt(p.get('od_in'), 3, ' in')}",
            f"  ID              : {_fmt(p.get('id_in'), 3, ' in')}",
            f"  Wall            : {_fmt(p.get('wall_in'), 3, ' in')}",
            f"  Grade           : {p.get('grade') or '—'}",
            f"  Weight          : {_fmt(p.get('weight_ppf'), 1, ' ppf')}",
            f"  Yield           : {_fmt(p.get('yield_psi'), 0, ' psi')}",
            f"  Yield @ temp    : {_fmt(p.get('yield_at_temp_psi'), 0, ' psi')}",
            f"  Internal P      : {_fmt(p.get('internal_pressure_psi'), 0, ' psi')}",
            f"  External P      : {_fmt(p.get('external_pressure_psi'), 0, ' psi')}",
            f"  Axial tension   : {_fmt(p.get('axial_tension_lbf'), 0, ' lbf')}",
            f"  Conn burst/coll/tens : "
            f"{_fmt(p.get('connection_burst_psi'), 0)} / "
            f"{_fmt(p.get('connection_collapse_psi'), 0)} / "
            f"{_fmt(p.get('connection_tension_lbf'), 0)}",
            "",
            "STORED RESULT (historical claim)",
            f"  Burst rating    : {_fmt(s.summary.get('burst_rating_psi'), 0, ' psi')}",
            f"  Collapse rating : {_fmt(s.summary.get('collapse_rating_psi'), 0, ' psi')}",
            f"  Body yield      : {_fmt(s.summary.get('pipe_body_yield_lbf'), 0, ' lbf')}",
            f"  Governing burst : {_fmt(s.summary.get('governing_burst_psi'), 0, ' psi')}",
            f"  Governing coll. : {_fmt(s.summary.get('governing_collapse_psi'), 0, ' psi')}",
            f"  Governing tens. : {_fmt(s.summary.get('governing_tension_lbf'), 0, ' lbf')}",
            "",
            "NOTE: casing 'database' presets are hard-coded (no durable catalog "
            "identity), so no reference fingerprint is claimed — the frozen "
            "numeric snapshot alone reproduces this run.",
        ]
        return "\n".join(lines)
